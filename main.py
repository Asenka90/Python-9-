...
def draw_tic_tac_toe(str, items):
    for(i)in range(3):
        for(j)in range(3):
            if items [i][j] == "0":
                pygame.draw.circle(str,(255,0,0), (j*100+50,i *100+50),45)
            elif items [i][j] == "x":
                pygame.draw.line(str, (0,0,255),(j*100+5,i *100+5),(j*100+95,i *100+95),3)
                pygame.draw.line(str, (0,0,255),(j*100+95,i *100+5),(j*100+5,i *100+95),3)

SCREEN_SIZE=(300,300)

window = pygame.display.set_mode(SCREEN_SIZE)
screer = pygame.Surface(SCREEN_SIZE)

pygame.display.set_caption("Крестики-нолики")
scrreen.fill((225,225,225))
...
while mainloop:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUYYONDOWN and not game_over:
            pos = pygame.mouse.get_pos()
            if field[pos[1]//100][pos[0]//100]  == "":
                field[pos[1]//100][pos] [pos[0]//100]  = "x"
                x,y = randome.randint(0,2), randome.randint(0,2)
                while field[x][y] != "":
                    x,y = randome.randint(0,2), randome.randint(0,2)  
                field[x][y]="0"
            ...
            elif field[0].count("x") + field[0].count("0")+ field[1].count("0") + \
